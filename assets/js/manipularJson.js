...
$(function () {
    iniciaRecursividade();
    //$("#dhInicializacao").html(new Date(nDia, nMes, nAno));//new Date();
    now = new Date;
    console.log(now);
    now.setMonth(now.getMonth() + 1);
    $("#dhInicializacao").html(now.getDate ()+"/"+now.getMonth()+"/"+now.getFullYear () +" - "+ now.getHours()+":"+now.getMinutes()+":"+now.getSeconds());
});
...
function carregarDados(arrJson, idTabela) {
    ...
    $.each(arrJson, function (key, val) {
        if (key != "99999999999999" && val <= 2000) {
            str += '<tr class="odd gradeC"><td>' + key + '</td><td>' + val + '</td></tr>';
        } else if (key != "99999999999999" && val > 2000) {
