...
function tratarJsonCNPJ($arrayJson){
    //echo $arrayJson;
    $jsonTratado = "{";
    foreach ($arrayJson as $indiceDoArray => $valorDoArray) {
        if($indiceDoArray == "nome"){
            $jsonTratado .= "\"$indiceDoArray\":\"$valorDoArray\",";
        }
    }
    $jsonTratado = substr($jsonTratado, 0, -1) . "}"; 
...
if (@$_GET["url"]==1) {
   ...
   tratarJsonMemoria(lerJason("http://redis.oobj-dfe.com.br/memoriaDisponivel"));
    ...
    //echo "{\"nome\":\"SINDICATO DA INDUSTRIA DA CONSTRUCAO NO ESTADO DE GOIAS\"}";
    tratarJsonCNPJ(lerJason("https://www.receitaws.com.br/v1/cnpj/{$_GET["cnpj"]}"));
